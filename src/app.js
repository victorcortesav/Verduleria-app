...
const BANCO_ICONOS = [
  ...
  ["Pollo","🍗"],["Pollo entero","🐔"],["Carne de vacuno","🥩"],["Costillar","🥩"],
  ["Cerdo","🥩"],["Carne molida","🥩"],["Tocino","🥓"],["Jamón","🍖"],
  ["Salchichas","🌭"],["Longaniza","🌭"],["Chorizo","🌭"],["Pescado","🐟"],
  ["Salmón","🐟"],["Atún fresco","🐟"],["Camarones","🍤"],["Mariscos","🦐"],

  // Lácteos y refrigerados
  ["Crema de leche","🥛"],["Queso rallado","🧀"],["Queso crema","🧀"],
  ["Mantequilla sin sal","🧈"],["Leche condensada","🥫"],["Leche en polvo","🥛"],
  ["Margarina","🧈"],["Leche de almendras","🥛"],

  // Panadería y pastelería
  ["Pan de molde","🍞"],["Hallulla","🍞"],["Marraqueta","🥖"],["Baguette","🥖"],
  ["Croissant","🥐"],["Queque","🍰"],["Torta","🎂"],["Galletas","🍪"],
  ["Donut","🍩"],["Pretzel","🥨"],["Tortilla","🫓"],["Pan integral","🍞"],["Bagel","🥯"],

  // Bebidas
  ["Agua mineral","💧"],["Bebida gaseosa","🥤"],["Jugo de fruta","🧃"],
  ["Jugo en caja","🧃"],["Cerveza","🍺"],["Vino tinto","🍷"],["Vino blanco","🥂"],
  ["Espumante","🍾"],["Pisco","🥃"],["Whisky","🥃"],["Café molido","☕"],
  ["Té en bolsitas","🍵"],["Bebida energética","🥤"],

  // Snacks y dulces
  ["Papas fritas (snack)","🍟"],["Chocolate en barra","🍫"],["Caramelos","🍬"],
  ["Chicles","🍬"],["Palomitas de maíz","🍿"],["Barra de cereal","🍫"],
  ["Helado","🍦"],["Paleta helada","🍦"],

  // Cereales y desayuno
  ["Cereal de desayuno","🥣"],["Avena instantánea","🥣"],["Mermelada","🍯"],
  ["Manjar","🍯"],["Crema de avellana","🍫"],

  // Conservas y enlatados
  ["Atún en lata","🥫"],["Choclo en lata","🥫"],["Arvejas en lata","🥫"],
  ["Durazno en conserva","🥫"],["Salsa de tomate","🥫"],["Puré de tomate","🥫"],
  ["Pickles","🥒"],

  // Congelados
  ["Verduras congeladas","🧊"],["Papas fritas congeladas","🍟"],
  ["Pizza congelada","🍕"],["Empanadas congeladas","🥟"],

  // Pastas y abarrotes
  ["Pasta larga","🍝"],["Ravioles","🥟"],["Lasaña","🍝"],["Polenta","🌽"],["Salsa pesto","🥫"],

  // Limpieza del hogar
  ["Detergente","🧴"],["Lavaloza","🧴"],["Cloro","🧴"],["Desinfectante","🧴"],
  ["Esponja","🧽"],["Papel higiénico","🧻"],["Toalla de papel","🧻"],
  ["Bolsas de basura","🗑️"],["Escoba","🧹"],["Suavizante de ropa","🧴"],

  // Cuidado personal e higiene
  ["Shampoo","🧴"],["Jabón","🧼"],["Pasta de dientes","🪥"],["Cepillo de dientes","🪥"],
  ["Desodorante","🧴"],["Pañales","🍼"],["Protector solar","🧴"],["Afeitadora","🪒"],

  // Bebé
  ["Leche de fórmula","🍼"],["Toallitas húmedas","🧻"],

  // Mascotas
  ["Comida para perro","🐶"],["Comida para gato","🐱"],["Arena para gato","🐾"],

  // Condimentos y salsas
  ["Mayonesa","🥫"],["Ketchup","🍅"],["Mostaza","🌭"],["Salsa soya","🍶"],
  ["Salsa de ají","🌶️"],["Aceite de oliva","🫒"],["Aceite vegetal","🍶"],
];
...
function renderProductos() {
  ...
  const productosOrdenados = [...productos].sort((a, b) => {
    ...
    return diff !== 0 ? diff : a.id - b.id; // empate: mantiene orden de creación
  });
...
document.getElementById("btn-finalizar-venta").addEventListener("click", () => {
  ...
  // Listo para la siguiente venta de inmediato - no hace falta tocar nada más
  carrito = [];
  actualizarBarraCarrito();
  renderProductos(); // reordena según los más vendidos, incluyendo esta venta
...
async function guardarArchivo(nombreArchivo, blob) {
  try {
    ...
    // Respaldo: descarga estándar del navegador (por si el plugin no está disponible)
    ...
    alert("No se pudo guardar con el método principal. Se intentó una descarga alternativa - revisa la carpeta Descargas.");
  }
}
...
document.getElementById("btn-enviar-correo").addEventListener("click", async () => {
  ...
  try {
    const base64 = await blobToBase64(blob);
    // Se guarda en caché (temporal) solo para poder compartirlo de inmediato
    const resultado = await Filesystem.writeFile({
      path: nombreArchivo,
      data: base64,
    ...
    alert("No se pudo abrir el selector de apps para compartir. Prueba con 'Descargar Excel' y adjúntalo manualmente desde tu app de correo.");
  }
});
